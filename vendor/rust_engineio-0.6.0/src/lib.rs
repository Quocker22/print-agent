...
/// print-agent (vá cục bộ, xem VA-CUC-BO.md): hạn cho bước DỰNG transport
/// websocket (TCP + TLS + HTTP 101). Bản gốc 0.6.0 không có hạn → mạng chết giữa
...
pub(crate) fn loi_qua_han_ket_noi() -> Error {
    Error::IncompleteIo(std::io::Error::new(
        std::io::ErrorKind::TimedOut,
        "dung ket noi websocket qua han (TCP/TLS/nang cap khong tra loi)",
    ))
}
