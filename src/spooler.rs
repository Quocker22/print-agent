    ...
    loop {
        ...
        for q in quan_sat {
            if let Some(kl) = bo_suy.them(q) {
                return ket_thuc(sp, job_id, kl, &bo_suy, su_co_da_thay, con_trong_hang_doi, bao);
            }
        }
        if !con_thoi_gian() {
...
/// máy in sau khi job rời đi (R5c), xoá job ứng viên (R2), báo job còn trong
/// hàng đợi để theo dõi tiếp (R3).
fn ket_thuc(
    sp: &mut dyn Spooler,
    job_id: &str,
    kl: KetLuan,
